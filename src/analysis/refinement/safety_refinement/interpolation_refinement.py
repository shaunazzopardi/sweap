...
def safety_refinement_seq_int(
    ...
    if allow_user_input:
        ...
        new_state_preds = [
            x
            for x in new_state_preds
            if not is_tautology(x, symbol_table)
            and not is_contradictory(x, symbol_table)
        ]

    state_predicates = set()
    state_predicates.update(
        s.pred for s in predicate_abstraction.get_state_predicates()
    )
    state_predicates.update(
        s for c in predicate_abstraction.v_to_chain_pred.values() for s in c.chain
    )
    ...
    logging.info(
        ...
        + ", ".join(
            [
                str(p)
                for p in new_all_preds
                if p not in state_predicates and neg(p) not in state_predicates
            ]
        )
    )
