        ...
        with NamedTemporaryFile("w", suffix=".c", delete=False) as tmp:
            ...
            try:
                ...
                if "Verification result: UNKNOWN" in out:
                    ...
                    try:
                        term_arg = (
                            out.split("Termination argument consisting of:")[1]
                            .strip()
                            .split("\n")
                        )
                        ...
                        ranking_function = (
                            term_arg[0]
                            .replace("Ranking function ", "")
                            .replace("main::", "")
                        )
                        ranking_function = ranking_function.removeprefix(
                            ranking_function.split(") = ")[0]
                        ).removeprefix(") = ")
                        ...
                        if "-nested ranking function" in ranking_function:
                            logging.info(
                                "Termination proved, but could only find a nested ranking function, "
                                "I do not deal with this yet unfortunately for you..: "
                                + str(ranking_function)
                            )
                            return True, None

                        logging.info(main_function + "\n" + out)
                        return True, (
                            string_to_math_expression(ranking_function).simplify(),
