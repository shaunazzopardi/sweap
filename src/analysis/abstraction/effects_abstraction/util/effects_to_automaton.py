...
def effects_to_explicit_automaton_abstraction(predAbs):
    ...
    for t in predAbs.init_program_trans:
        ...
        for _, E in predAbs.abstract_guard_env_disjuncts[t]:
            ...
            for p in predAbs.get_state_predicates():
                if sat(
                    conjunct_formula_set(E | {p, t_f}),
                    predAbs.get_program().symbol_table,
                ):
                    next.add(p)
                else:
                    next.add(neg(p))
            ...
            abs_trans = Transition(
                ...
                tgt,
            ).complete_outputs(predAbs.get_program().out_events)
                ...
                for P in Ps:
                    ...
                    if len(unused_invars) > 0:
                        for p_set in powerset(unused_invars):
                            ...
                            p_set_closed_src = p_set_closed_src | {
                                neg(p)
                                for p in predAbs.get_state_predicates()
                                if p not in p_set_closed_src
                                and neg(p) not in p_set_closed_src
                            }
                            p_set_closed_tgt = p_set | nextPs_with_constants | invars
                            p_set_closed_tgt |= {
                                neg(p)
                                for p in predAbs.get_state_predicates()
                                if p not in p_set_closed_tgt
                                and neg(p) not in p_set_closed_tgt
                            }
                            ...
                            abs_trans = Transition(
                                ...
                                [],
                                t.output
                                + [stringify_pred_take_out_neg(t) for t in tran_preds],
                                tgt,
                            ).complete_outputs(predAbs.get_program().out_events)
                            safe_update_set_vals(
                                new_env_to_program_transitions, abs_trans, {t}
                            )
                            safe_update_set_vals(
                                predAbs.state_to_env_transitions,
                                src,
                        ...
                        safe_update_set_vals(
                            predAbs.state_to_env_transitions, src, {abs_trans}
                        )
                ...
                for P in Ps:
                    ...
                    if len(unused_invars) > 0:
                        for p_set in powerset(unused_invars):
                            ...
                            safe_update_set_vals(
                                new_con_to_program_transitions, abs_trans, {t}
                            )
                            safe_update_set_vals(
                                predAbs.state_to_con_transitions,
                                src,
                        ...
                        safe_update_set_vals(
                            predAbs.state_to_con_transitions, src, {abs_trans}
                        )
    ...
    one_step_abstraction = Program(
        ...
        program.con_events,
        program.out_events
        + [stringify_pred(t) for t in predAbs.get_transition_predicates()],
        False,
        preprocess=False,
    )
