...
def relevant_pred(transition, relevant_preds, predicate):
    ...
    if any(
        True for v in predicate.variablesin() if v in transition.condition.variablesin()
    ):
        return True
    # check if used in action (without identities)
    elif any(
        True
        for v in predicate.variablesin()
        if any(
            True
            for act in transition.action
            if act.left != act.right and (v == act.left or v in act.right.variablesin())
        )
    ):
        return True
    elif any(
        True
        for v in predicate.variablesin()
        if any(True for p in relevant_preds if v in p.variablesin())
    ):
        return True
    else:
        return False
...
def merge_transitions(transitions: [Transition], symbol_table, to_program_transitions):
    ...
    for transition in transitions:
        key = (
            str(transition.src)
            + str(conjunct_formula_set(sorted(transition.output, key=lambda x: str(x))))
            + str(transition.tgt)
        )
    ...
    for key in partitions.keys():
        ...
        conditions = disjunct_formula_set(
            sorted([t.condition for t in trans_here], key=lambda x: str(x))
        )
        ...
        if not check(Not(conditions_smt)):
            ...
            conditions_simplified_fnode = (
                conditions_smt  # simplify(conditions_smt)#.simplify()
            )
        ...
        try:
            new_tran = Transition(
                trans_here[0].src,
                fnode_to_formula(conditions_simplified_fnode),
                [],
                trans_here[0].output,
                trans_here[0].tgt,
            )
...
def tran_and_state_preds_after_con_env_step(env_trans: Transition):
    if True:
        src_tran_preds = [
            p
            for p in env_trans.src.predicates
            if [] != [v for v in p.variablesin() if v.name.endswith("_prev")]
        ]
        tgt_tran_preds = [
            p
            for p in env_trans.tgt.predicates
            if [] != [v for v in p.variablesin() if v.name.endswith("_prev")]
        ]
