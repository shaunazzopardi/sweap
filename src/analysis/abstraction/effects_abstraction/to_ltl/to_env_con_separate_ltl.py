...
def empty_abstraction(predicate_abstraction: EffectsAbstraction):
    return (
        len(predicate_abstraction.state_predicates) == 0
        and len(predicate_abstraction.transition_predicates) == 0
    )


def to_env_con_separate_ltl(predicate_abstraction: EffectsAbstraction):
    rename_pred = lambda x: (
        label_pred(x, predicate_abstraction.get_all_preds())
        if x
        not in predicate_abstraction.program.env_events
        + predicate_abstraction.program.con_events
        and negate(x)
        not in predicate_abstraction.program.env_events
        + predicate_abstraction.program.con_events
        else x
    ...
    for t in predicate_abstraction.init_program_trans:
        for E, effect in predicate_abstraction.abstract_effect[t].items():
            if len(effect.keys()) > 1:
                raise Exception(
                    "init transition " + str(t) + " has more than one possible effect"
                )
            ...
            E_effects.update(
                {
                    rename_pred(p)
                    for p in predicate_abstraction.abstract_effect_constant[t]
                }
            )
            E_effects.update(
                {
                    rename_pred(p)
                    for p in predicate_abstraction.abstract_effect_tran_preds_constant[
                        t
                    ]
                }
            )
            ...
            pred_next[frozenset(E_effects)].add(
                conjunct_formula_set([Variable(t.tgt)] + list(E) + t.output)
            )
            init_transitions.extend(
                [
                    conjunct_formula_set(
                        list(E_effects) + [Variable(t.tgt)] + list(E) + t.output
                    )
                ]
            )
    ...
    for E_effects in pred_next.keys():
        init_transition_ltls.append(
            conjunct(
                conjunct_formula_set(conjunct(preds, X(preds)) for preds in E_effects),
                disjunct_formula_set(
    ...
    for env_trans in predicate_abstraction.abstract_guard_env.keys():
        ...
        if env_trans in predicate_abstraction.init_program_trans:
            continue
        invar_preds_effects = []

        if env_trans in predicate_abstraction.abstract_effect_invars.keys():
            invar_preds_effects += [
                iff(rename_pred(p), X(rename_pred(p)))
                for p in set(predicate_abstraction.abstract_effect_invars[env_trans])
            ]

        if env_trans in predicate_abstraction.abstract_effect_constant.keys():
            invar_preds_effects += [
                X(rename_pred(p))
                for p in set(predicate_abstraction.abstract_effect_constant[env_trans])
            ]

        if (
            env_trans
            in predicate_abstraction.abstract_effect_tran_preds_constant.keys()
        ):
            invar_preds_effects += [
                X(rename_pred(p))
                for p in set(
                    predicate_abstraction.abstract_effect_tran_preds_constant[env_trans]
                )
            ]
        ...
        for E, effect in predicate_abstraction.abstract_effect[env_trans].items():
            ...
            new_effects = [
                (set(now).union(E), nexts) for now, nexts in inverted_effects
            ]
        ...
        formula = flatten_effects(
            all_effects, predicate_abstraction.state_predicates, rename_pred
        )
        ...
        effect_formula = conjunct(
            conjunct(pred_effect_formula, invar_preds_formula), output_formula
        )

        next = conjunct(
            effect_formula, X(conjunct_formula_set([Variable(env_trans.tgt)]))
        )
    ...
    for con_trans in predicate_abstraction.abstract_guard_con.keys():
        ...
        if con_trans in predicate_abstraction.abstract_effect_invars.keys():
            invar_preds_effects += [
                iff(rename_pred(p), X(rename_pred(p)))
                for p in set(predicate_abstraction.abstract_effect_invars[con_trans])
            ]

        if con_trans in predicate_abstraction.abstract_effect_constant.keys():
            invar_preds_effects += [
                X(rename_pred(p))
                for p in set(predicate_abstraction.abstract_effect_constant[con_trans])
            ]
        ...
        if (
            ...
            invar_preds_effects += [
                X(rename_pred(p))
                for p in set(
                    predicate_abstraction.abstract_effect_tran_preds_constant[con_trans]
                )
            ]
        ...
        pred_effect_formula = disjunct_formula_set(pred_effects)
        output_formula = conjunct_formula_set(
            [X(neg(o)) for o in predicate_abstraction.program.out_events]
        )
        effect_formula = conjunct(
            conjunct(pred_effect_formula, invar_preds_formula), output_formula
        )
        ...
        next = conjunct(
            effect_formula, X(conjunct_formula_set([Variable(con_trans.tgt)]))
        )
    ...
    _env_transition_ltl = [
        (
            G(
                implies(
                    conjunct(conjunct(neg(init_state), env), src),
                    disjunct_formula_set(env_transition_ltl[src]),
                )
            )
        )
        for src in env_transition_ltl.keys()
    ]
    _con_transition_ltl = [
        (
            G(
                (
                    implies(
                        conjunct(conjunct(neg(init_state), con), src),
                        disjunct_formula_set(con_transition_ltl[src]),
                    )
                )
            )
        )
        for src in con_transition_ltl.keys()
    ]

    program = predicate_abstraction.get_program()
    at_least_and_at_most_one_state = UniOp(
        "G",
        conjunct_formula_set(
            [
                BiOp(
                    ...
                    "<=>",
                    conjunct_formula_set(
                        [neg(Variable(r)) for r in program.states if r != q]
                    ),
                )
                for q in program.states
                if "loop" not in str(q)
            ]
        ),
    ).to_nuxmv()
    ...
    return (
        [init_transition_ltl, at_least_and_at_most_one_state]
        + _env_transition_ltl
        + _con_transition_ltl
    )


def to_env_con_separate_ltl_organised_by_pred_effects(
    ...
    for t in predicate_abstraction.init_program_trans:
        for E, effect in predicate_abstraction.abstract_effect[t].items():
            ...
            E_effects.update(
                {
                    neg(rename_pred(p))
                    for p in predicate_abstraction.abstract_effect_invars[t]
                }
            )
            E_effects.update(
                {
        ...
        for E, effect in predicate_abstraction.abstract_effect[env_trans].items():
            ...
            for next_pred_state, pred_states in effect.items():
                E_now = disjunct_formula_set(
                    [
                        conjunct_formula_set([rename_pred(p) for p in pred_state])
            ...
            pred_effects.extend(
                [
                    conjunct(
                        ...
                        disjunct_formula_set(E_effects),
                    )
                ]
            )
    ...
    for con_trans in predicate_abstraction.abstract_guard_con.keys():
        ...
        for E, effect in predicate_abstraction.abstract_effect[con_trans].items():
            ...
            pred_effects += [
                conjunct(conjunct_formula_set(E), disjunct_formula_set(E_effects))
            ]
        pred_effect_formula = disjunct_formula_set(pred_effects)
        output_formula = conjunct_formula_set(
            [X(neg(o)) for o in predicate_abstraction.program.out_events]
...
def abstract_ltl_problem(
    ...
    pred_props = (
        [Variable(s) for s in program.states] + list(predicate_vars) + [env, init_state]
    )
    ...
    original_assumptions_expanded = [
        expand_ltl_to_env_con_steps(a, original_LTL_problem.env_props)
        for a in original_LTL_problem.assumptions
    ]

    original_guarantees_expanded = [
        expand_ltl_to_env_con_steps(g, original_LTL_problem.env_props)
        for g in original_LTL_problem.guarantees
    ]

    assumptions = (
        turn_logic
        + effects_abstraction.ranking_constraints
        + ltl_abstraction
        + original_assumptions_expanded
    )
...
def expand_ltl_to_env_con_steps_with_until(formula: Formula, env_events: [Variable]):
    rewritten_formula = formula.replace_vars(
        lambda x: (
            x
            if not isinstance(x, Variable)
            else (
                U(con, conjunct(env, x))
                if x in env_events
...
def expand_ltl_to_env_con_steps(formula: Formula, env_events: [Variable]):
    if isinstance(formula, Value) or isinstance(formula, MathExpr):
        ...
        if formula.op == "X":
            ...
            return G(
                implies(env, expand_ltl_to_env_con_steps(formula.right, env_events))
            )
        else:
            return UniOp(
                formula.op,
