...
game=$(tslmt2rpg --prune --rules-disable-precise-deduction --muval-caller call-muval.sh  --muval-timeout 20 < $INPUT) 
echo "Run rpgsolve at $(date +%H:%M:%S)"
res=$(echo "$game" | rpgsolve --disable-log --generate-program)
echo "$res"
