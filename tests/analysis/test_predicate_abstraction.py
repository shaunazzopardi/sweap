...
from analysis.abstraction.interface.predicate_abstraction import abstraction_to_ltl_with_turns, predicate_abstraction_init
...
class Test(TestCase):
    def test_abstraction_to_ltl_with_turns(self):
        with open('../../examples/parallel/arbiter/program.prog') as file:
            ...
            out = model_checker.invar_check(nuxmv_model, str(ltl_abstraction), None, False)
            if not out[0]:
                print(out[1])
