...
TOOLS := rpgsolve rpgsolve-syn rpg-stela sweap sweap-lazy sweap-nobin tslmt2rpg tslmt2rpg-syn sweap-rpg sweap-tsl sweap-semml issy-rpg sweap-dual issy-tsl
...
$(ISSY_RPG_LOGS): cmd =			issy --synt --rpg
$(ISSY_TSL_LOGS): cmd =			issy --synt --tslmt 
...
clean-timeouts: confirm
	...
	-@tail -n2 $(ISSY_RPG_LOGS) 2>/dev/null | grep -B1 124 | grep "==>" | cut -d ' ' -f2 | xargs rm -v 2>/dev/null || true
