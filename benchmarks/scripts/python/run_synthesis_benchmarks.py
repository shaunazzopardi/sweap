...
if not os.path.exists(csv_dir):
    os.makedirs(csv_dir)
csv_name = "synthesis_results-verif2-15Apr-dual.csv"
...
def test_synthesis():
    ...
    print(f"SWEAP dual fallback enabled: {DUAL_FALLBACK_ENABLED}")
        ...
        for file in files:
            if file.endswith(".prog") and file not in ignore:
                ...
                config.Config.getConfig().log = (
                    logdir + "/sweap/" + csv_name.replace(".csv", "")
                )
                            ...
                            if success:
                                ...
                                try:
                                    ...
                                    while (
                                        ...
                                        # Allow exactly one retry with fallback mode.
