...
def test_synthesis():
    ...
    csv_dir = os.path.join(dirname, "../../rpgsolve/results")
    if not os.path.exists(csv_dir):
        os.makedirs(csv_dir)
    csv_name = "synthesis_results-15Apr-verifv2.csv"
    ...
    print(f"RPG dual fallback enabled: {DUAL_FALLBACK_ENABLED}")
    ...
    for root, dirs, files in os.walk(benchmarks_dir):
        for file in files:
            if file.endswith(".rpg") and file not in ignore:
                import gc
                ...
                config.Config.getConfig().log = (
                    logdir + "/rpg/" + csv_name.replace(".csv", "")
                )
...
def test_parsing():
    ...
    for file in os.listdir(benchmarks_dir):
        if "taxi-service" in file:
            print("taxi-service takes a really long time.")
            continue
